...
public class GSMASettings extends PreferenceActivity implements
    ...
    private static final String GSMA_TRANSACTION_COMPATIBLE = "NfcService.GSMA.TRANSACTION.Compatible";
    ...
    public static final String MODE_HCE = "mode.hce";
    public static final String MODE_P2P = "mode.p2p";
    ...
    private void loadGSMATransactionCompatible(SwitchPreference gsma_o_compatible) {
        boolean allow = Settings.Global.getInt(getContentResolver(), GSMA_TRANSACTION_COMPATIBLE, 1) != 0;
        gsma_o_compatible.setChecked(allow);
    }
    ...
    private void loadNfcModes(CheckBoxPreference reader, CheckBoxPreference hce, CheckBoxPreference p2p) {
        if (null == reader || null == hce || null == p2p) {
            return;
        }
        reader.setChecked(1 == mNxpNfcAdapter.getModeFlag(1) ? true : false);
        hce.setChecked(1 == mNxpNfcAdapter.getModeFlag(2) ? true : false);
        p2p.setChecked(1 == mNxpNfcAdapter.getModeFlag(4) ? true : false);
    }
    ...
    private void loadItems() {
        ...
        SwitchPreference gsma_o_compatible = createSwitchPreferenceIfNeeded(screen,
                GSMA_TRANSACTION_COMPATIBLE, "GSMA transaction compatible", "com.gsma.services.nfc.action.TRANSACTION_EVENT broadcast compatible");
        ...
        CheckBoxPreference reader = null;
        CheckBoxPreference hce = null;
        CheckBoxPreference p2p = null;
        if (-1 != mNxpNfcAdapter.getModeFlag(1)) {
            reader = createCheckBoxPreferenceIfNeeded(screen, MODE_READER, "Reader Mode", "Set NFC Reader Mode");
            hce = createCheckBoxPreferenceIfNeeded(screen, MODE_HCE, "HCE Mode", "Set NFC HCE Mode");
            p2p = createCheckBoxPreferenceIfNeeded(screen, MODE_P2P, "P2P Mode", "Set NFC P2P Mode");
        }
        ...
        loadGSMATransactionCompatible(gsma_o_compatible);
        loadUpdateRoutingTableJIT(jltUpdateRT);
        loadServicesFilter(filter);
        loadNfcModes(reader, hce, p2p);
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference instanceof CheckBoxPreference) {
            ...
            } else if (MODE_READER.equals(key)) {
                mNxpNfcAdapter.setModeFlag(1, on ? 1 : 0);
            } else if (MODE_HCE.equals(key)) {
                mNxpNfcAdapter.setModeFlag(2, on ? 1 : 0);
            } else if (MODE_P2P.equals(key)) {
                mNxpNfcAdapter.setModeFlag(4, on ? 1 : 0);
            }
            ...
            } else if (GSMA_TRANSACTION_COMPATIBLE.equals(key)) {
                Settings.Global.putInt(getContentResolver(), GSMA_TRANSACTION_COMPATIBLE, allow ? 1 : 0);
