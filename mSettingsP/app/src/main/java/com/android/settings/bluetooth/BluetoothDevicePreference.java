 ...
 */
//porting androidP by jiabin.zheng.hz for XR7177433 on 2018/12/11
...
public final class BluetoothDevicePreference extends GearPreference implements
    ...
    private AlertDialog mActiveDialog; //Added by weijun.pan for XR7236931 on 2019/01/03
    ...
    public void onDeviceAttributesChanged() {
        ...
        //Begin modified by jiabin.zheng.hz for XR7233909 on 2018/12/21
        //final Pair<Drawable, String> pair = com.android.settingslib.bluetooth.Utils
        final Pair<Drawable, String> pair = Utils
        //End modified by jiabin.zheng.hz for XR7233909 on 2018/12/21
    ...
    void onClicked() {
        ...
        if (mCachedDevice.isConnected()) {
            //Begin modified by jiabin.zheng.hz for XR7177433 on 2018/12/11
//            if (!mCachedDevice.isActiveDevice(BluetoothProfile.A2DP)
//                    && !mCachedDevice.isActiveDevice(BluetoothProfile.HEADSET)) {
            //Begin modified by weijun.pan for XR7236931 on 2019/01/03
            //    mCachedDevice.setActive();
            if (isAudioModeOngoingCall(context)) {
                if (mCachedDevice.isHfpDevice()
                        && !mCachedDevice.isActiveDevice(BluetoothProfile.HEADSET)) { //modified by weijun.pan for XR7311208  on 2019/01/09
                    askActive();
                }
            } else if (mCachedDevice.isA2dpDevice() 
                    && !mCachedDevice.isActiveDevice(BluetoothProfile.A2DP)) { //modified by weijun.pan for XR7311208  on 2019/01/09
                askActive();
            } //End modified by weijun.pan for XR7236931 on 2019/01/03
//            } else {
//                metricsFeatureProvider.action(context,
//                        MetricsEvent.ACTION_SETTINGS_BLUETOOTH_DISCONNECT);
//                askDisconnect();
//            }
            //End modified by jiabin.zheng.hz for XR7177433 on 2018/12/11
    ...
    // Begin added by weijun.pan for XR7236931 on 2019/01/03
    private void askActive() {
        Context context = getContext();
        String name = mCachedDevice.getName();
        if (TextUtils.isEmpty(name)) {
            name = context.getString(R.string.bluetooth_device);
        }
        String message = context.getString(R.string.bluetooth_active_device, name);
        String title = context.getString(R.string.bluetooth_active_title);

        DialogInterface.OnClickListener activeListener = new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss(); //Added by weijun.pan for XR P10029496 on 2019/01/16
                mCachedDevice.setActive();
            }
        };

        mActiveDialog = Utils.showDisconnectDialog(context,
                mActiveDialog, activeListener, title, Html.fromHtml(message));
    } // End added by weijun.pan for XR7236931 on 2019/01/03
}
