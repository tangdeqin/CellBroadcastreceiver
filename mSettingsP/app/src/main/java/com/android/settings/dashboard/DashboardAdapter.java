...
public class DashboardAdapter extends RecyclerView.Adapter<DashboardAdapter.DashboardItemHolder>
    ...
    void onBindSuggestionConditionHeader(final SuggestionAndConditionHeaderHolder holder,
        ...
        holder.itemView.setOnClickListener(v -> {
            ...
            notifyDashboardDataChanged(prevData);
            if (wasCollapsed || onlyHasConditionAndCollapsed(hasConditions, curMode)) {//Modified by miaoliu for XRP10029861 on 2019/1/23
                mRecyclerView.scrollToPosition(SUGGESTION_CONDITION_HEADER_POSITION);
            }
        });
    }

    //Begin added by miaoliu for XRP10029861 on 2019/1/23
    private boolean onlyHasConditionAndCollapsed(boolean hasConditions, int suggestionConditionMode){
        boolean onlyHasConditionAndCollapsed = false;
        if(mDashboardData != null){
            boolean hasSuggestions = false;
            if(mDashboardData.getSuggestions() != null){
                hasSuggestions = mDashboardData.getSuggestions().size() > 0;
            }
            onlyHasConditionAndCollapsed = !hasSuggestions
                && hasConditions
                && suggestionConditionMode != DashboardData.HEADER_MODE_FULLY_EXPANDED;
        }
        return onlyHasConditionAndCollapsed;
    }
    //End added by miaoliu for XRP10029861 on 2019/1/23
