...
import android.media.AudioSystem;//Add by ronghui.yi for task 7448330 data hub 20190219
...
//Begin modified by miaoliu for XR7107006 on 2018/11/30
import android.text.BidiFormatter;
import android.text.format.Formatter.BytesResult;
import android.text.format.Formatter;
//End modified by miaoliu for XR7107006 on 2018/11/30
// Begin added by weijun.pan for XR7220290 on 2018/12/14
import com.android.settings.bluetooth.BluetoothPairingDetail;
// End added by weijun.pan for XR7220290 on 2018/12/14
// Begin added by weijun.pan for XR7236931 on 2019/01/03
import com.android.settings.bluetooth.BluetoothSettings;
// End added by weijun.pan for XR7236931 on 2019/01/03

//begin add by zhixiong.liu.hz for task 7447417 data hub 20190214 
import java.util.Map;
import java.util.HashMap; 
import com.android.settingslib.drawer.Tile;
import tct.datahub.sdk.TctDataHubSdkManager;
import tct.datahub.sdk.TctDataHubConstants;
//end add by zhixiong.liu.hz for task 7447417 data hub 20190214  
...
public final class Utils extends com.android.settingslib.Utils {
    ...
    public static boolean assignDefaultPhoto(Context context, int userId) {
        ...
        UserManager um = (UserManager) context.getSystemService(Context.USER_SERVICE);
        Bitmap bitmap = getDefaultUserIconAsBitmap(context,userId);
    ...
    public static void startWithFragment(Context context, String fragmentName, Bundle args,
        ...
        if (resultTo == null) {
            //Begin deleted by miaoliu for XRP10029441 on 2019/1/15
            //intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);//Added by miaoliu for XRP10027035 on 2018/11/28
           //End deleted by miaoliu for XRP10029441 on 2019/1/15
            context.startActivity(intent);
        } else {
            resultTo.startActivityForResult(intent, resultRequestCode);
        }
    }
//Begin added by miaoliu for XRP10029441 on 2019/1/15
    public static void startWithFragmentNew(Context context, String fragmentName, Bundle args,
            Fragment resultTo, int resultRequestCode, int titleResId,
            CharSequence title, int metricsCategory) {
       startWithFragmentNew(context, fragmentName, args, resultTo, resultRequestCode,
                null /* titleResPackageName */, titleResId, title, false /* not a shortcut */,
                metricsCategory);
    }
    public static void startWithFragmentNew(Context context, String fragmentName, Bundle args,
 ...
 //End added by miaoliu for XRP10029441 on 2019/1/15
   ...
   private static boolean isProfileOf(UserManager um, UserHandle otherUser) {
     ...
     */
    public static Bitmap getDefaultUserIconAsBitmap(Context context,int userId) {
        Bitmap bitmap = null;
        // Try finding the corresponding bitmap in the dark bitmap cache
        bitmap = sDarkDefaultUserBitmapCache.get(userId);
        if (bitmap == null) {
            bitmap = UserIcons.convertToBitmap(UserIcons.getDefaultUserIcon(context.getResources() ,userId, false));
            // Save it to cache
    ...
    private static final String[] ENTRY_FRAGMENTS = {
            ...
            // End added by weijun.pan for XR7220290 on 2018/12/14
            BluetoothSettings.class.getName(), //Added by weijun.pan for XR7236931 on 2019/01/03
    };
    //End Modified by nanbing.zou for P10024859 on 2018-10-25
   ...
   //Begin added by miaoliu for XR7025065 on 2018/10/18
    public static Intent getMobileNetworkSettingsIntent(Context context){
            Intent intent = new Intent(Intent.ACTION_MAIN);
            //begin zhixiong.liu.hz for XR 7070928 20181116
            if (checkPackageExist(context, "com.tct.networksetting")){
                intent.setComponent(new ComponentName("com.tct.networksetting",
                        "com.tct.networksetting.MobileNetworkSettings"));
            }
            else 
            //end zhixiong.liu.hz for XR 7070928 20181116
    ...
    //Begin added by miaoliu for XR7107006 on 2018/11/30
    /**
     * Format byte value to readable string using IEC units.
     */
    public static String formatDataUsage(Context context, long byteValue) {
        final BytesResult res = Formatter.formatBytes(context.getResources(), byteValue,
            Formatter.FLAG_IEC_UNITS);
        int  resId = context.getResources().getIdentifier("fileSizeSuffix", "string", "android");
        if(resId > 0){      
            return BidiFormatter.getInstance().unicodeWrap(context.getString(
            resId, res.value, res.units));
        }else{     
            StringBuilder fileSizeString = new StringBuilder();
            fileSizeString.append(res.value).append(" ").append(res.units);
            return BidiFormatter.getInstance().unicodeWrap(fileSizeString.toString());
         }
    }

   //End added by miaoliu for XR7107006 on 2018/11/30
   
   
    //begin add by zhixiong.liu.hz for task 7447417 data hub 20190214
    
    /**
     * First settings level menu and out apk launch interface  data hub 
     */
    private static int mDataHubon = 0;
    private static int isDataHubOn(Context context){
        if(mDataHubon == 0){
            String value = getString(context,"default_datahub_enable","com.tct","off");
            if("on".equals(value)){
                mDataHubon = 1;
            }
            else{
            ...
            //Log.d(TAG,"value="+value+",mDataHubon="+mDataHubon);          
        }
        return  mDataHubon;
    }
    
    public static void  dataHubSendMain(Context context,Tile tile){
        if(isDataHubOn(context)<0){
            return;
        }        
        TctDataHubSdkManager mDataHub = TctDataHubSdkManager.getInstance(context);
        //String localClass = tile.metaData.getString(SettingsActivity.META_DATA_KEY_FRAGMENT_CLASS);
        String localClass = tile.intent.getComponent().getClassName();
        String eventId = "null";
        switch(localClass){
            //main level menu
            case "com.android.settings.Settings$NetworkDashboardActivity":
                eventId = "Settings_Network_Tap";
                break;
            case "com.android.settings.Settings$DisplaySettingsActivity":
                eventId = "Settings_Display_Tap";
                break;
            case "com.android.settings.Settings$SecuritySettingsActivity":
                eventId = "Settings_Security_Tap";
                break;
            case "com.android.settings.Settings$AdvancedFeaturesSettingsActivity":
                eventId = "Settings_Advanced_Tap";
                break;
            case "com.android.settings.Settings$AppAndNotificationDashboardActivity":
                eventId = "Settings_Apps_Tap";
                break;
            case "com.tct.onetouchbooster.ui.MainActivity":
                eventId = "Settings_SmartManager_Tap";
                break;
            case "com.android.settings.Settings$SoundSettingsActivity":
                eventId = "Settings_Sound_Tap";
                break;
            case "com.android.settings.Settings$UserAndAccountDashboardActivity":
                eventId = "Settings_Accounts_Tap";
                break;
            case "com.google.android.gms.app.settings.GoogleSettingsIALink":
                eventId = "Settings_Google_Tap";
                break;
            case "com.android.settings.Settings$AccessibilitySettingsActivity":
                eventId = "Settings_Accessibility_Tap";
                break;
            case "com.android.settings.Settings$SystemDashboardActivity":
                eventId = "Settings_System_Tap";
                break;
                
            //out apk   
            case "com.tct.fullscreen.NotchActivity":
                eventId = "Settings_Display_Notch_Tap";
                break;                
            case "com.tct.fullscreen.FullscreenActivity":
                eventId = "Settings_Display_FullScreen_Tap";
                break;
            case "com.tcl.faceunlock.FaceUnlockSettings":
                eventId = "Settings_Security_Face_Tap";
                break;
            case "com.tct.appclone.DualAppActivity":
                eventId = "Settings_Adanced_AppCloner_Tap";
                break;
            case "com.tct.onehandmode.OneHandModeActivity":
                eventId = "Settings_Adanced_OneHanded_Tap";
                break;
            case "com.tct.android.tctgamerecorder.deprecation.SettingsActivity":
                eventId = "Settings_Adanced_ScreenRecorder_Tap";
                break; 
            case "com.google.android.gms.security.settings.VerifyAppsSettingsActivity":
                eventId = "Settings_Security_GPProtect_Tap";
                break; 
            case "com.google.android.gms.security.settings.AdmSettingsActivity":
                eventId = "Settings_Security_FindMyDevice_Tap";
                break; 
            case "com.google.android.gms.update.SystemUpdateActivity":
                eventId = "Settings_Security_SecurityUpdate_Tap";
                break; 
            //"Settings_System_AppsUpdate_Tap";       
        }
        //Log.d(TAG,"localClass="+localClass+",eventId="+eventId);
        if(!eventId.equals("null")){
           mDataHub.trackEventInfo(TctDataHubConstants.TYPE_RECORD_BY_TIME,eventId);
        }
    }
    /**
     * Sub settings level menu data hub
     */
    public static void  dataHubSendSub(Context context,String subkey){
        if(isDataHubOn(context)<0){
            return;
        }
        TctDataHubSdkManager mDataHub = TctDataHubSdkManager.getInstance(context);
        ...
        switch(subkey){
            //NetworkDashboard
            case "toggle_wifi":
               eventId = "Settings_Network_WiFi_Tap";
               break;
            case "toggle_bluetooth":
               eventId = "Settings_Network_BT_Tap";
               break;
            case "mobile_network_settings":
               eventId = "Settings_Network_Callular_Tap";
               break;
            case "toggle_nfc_master":
               eventId = "Settings_Network_NFC_Tap";
               break;
            case "dashboard_tile_pref_com.android.settings.Settings$SimSettingsActivity":
               eventId = "Settings_Network_SIM_Tap";
               break;
            case "dashboard_tile_pref_com.android.settings.Settings$DataUsageSummaryActivity":
               eventId = "Settings_Network_Data_Tap";
               break;
            case "tether_settings":
               eventId = "Settings_Network_Hotspot_Tap";
               break;
            case "vpn_settings":
               eventId = "Settings_Network_VPN_Tap";
               break;
            case "toggle_airplane":
               eventId = "Settings_Network_Airplane_Tap";
               break;
            case "usb_mode":
               eventId = "Settings_Network_USB_Tap";
               break;
            case "dashboard_tile_pref_com.android.settings.Settings$PrintSettingsActivity":
               eventId = "Settings_Network_Printing_Tap";
               break;

            //DisplaySettings
            case "default_home":
               eventId = "Settings_Display_Home_Tap";
               break;
            case "navigationbar":
               eventId = "Settings_Display_NaviBar_Tap";
               break;
            case "status_bar":
               eventId = "Settings_Display_StatusBar_Tap";
               break;
            case "brightness":
               eventId = "Settings_Display_Brightness_Tap";
               break;
            case "night_display":
               eventId = "Settings_Display_EyeComfort_Tap";
               break;
            case "reading_mode":
               eventId = "Settings_Display_ReadingMode_Tap";
               break;
            case "auto_brightness":
               eventId = "Settings_Display_AdaptiveBrightness_Tap";
               break;
            case "wallpaper":
               eventId = "Settings_Display_Wallpaper_Tap";
               break;
            case "auto_rotate":
               eventId = "Settings_Display_AutoRotate_Tap";
               break;
            case "screen_timeout":
               eventId = "Settings_Display_Sleep_Tap";
               break;
            case "font_size":
               eventId = "Settings_Display_FontSize_Tap";
               break;
            case "screensaver":
               eventId = "Settings_Display_ScreenSaver_Tap";
               break;               
            case "led_indictor":
               eventId = "Settings_Display_NotiLED_Tap";
               break;
            //
            //security
            case "unlock_set_or_change":
               eventId = "Settings_Security_ScreenLock_Tap";
               break;
            case "fingerprint_settings_fragment":
               eventId = "Settings_Security_Fingerprint_Tap";
               break;
            case "apps_lock":
               eventId = "Settings_Security_AppsLock_Tap";
               break;
            case "lockscreen_preferences":
               eventId = "Settings_Security_LockScrPre_Tap";
               break;
            case "location":
               eventId = "Settings_Security_Location_Tap";
               break;
            case "show_password":
               eventId = "Settings_Security_PassVisible_Tap";
               break;
            case "manage_device_admin":
               eventId = "Settings_Security_DeviceAdminApps_Tap";
               break;
            case "sim_lock_settings":
               eventId = "Settings_Security_SIMCardLock_Tap";
               break;
            case "encryption_and_credential":
               eventId = "Settings_Security_Encryption_Tap";
               break;
            case "screen_pinning_settings":
               eventId = "Settings_Security_ScreenPinning_Tap";
               break;
            case "trust_agents":
               eventId = "Settings_Security_SmartLock_Tap";
               break;    
            case "manage_trust_agents":
               eventId = "Settings_Security_TrustAgents_Tap";
               break;               
            case "usage_access":
               eventId ="Settings_Security_AppsWithUsageAccess_Tap";
               break;
  
            //AdvancedFeatures
            case "func_settings":
               eventId = "Settings_Adanced_Func_Tap";
               break;
            case "gesture_settings":
               eventId = "Settings_Adanced_Gestures_Tap";
               break;

            //Apps
            case "manage_perms":
               eventId = "Settings_Apps_AppPermissions_Tap";
               break;
            case "dashboard_tile_pref_com.android.settings.Settings$AdvancedAppsActivity":
               eventId = "Settings_Apps_DefaultApps_Tap";
               break;
            case "special_access":
               eventId = "Settings_Apps_SpecialApp_Tap";
               break;
           
            //sound
            //Begin add by ronghui.yi for task 7448330 data hub 20190218
            case "silent_mode_switch":
                eventId = "Sound_SilentMode_Tap";
                break;
            case "media_volume":
                eventId = "Sound_MediaVolume_Tap";
                break;
            case "alarm_volume":
                eventId = "Sound_AlarmVolume_Tap";
                break;
            case "ring_volume":
                eventId = "Sound_RingVolume_Tap";
                break;
            case "notification_volume":
                eventId = "Sound_NotificationVolume_Tap";
                break;
            case "headset_mode_preference":
                eventId = "Sound_Headset_Tap";
                break;
            case "zen_mode":
                eventId = "Sound_DoNotDisturb_Tap";
                break;
            case "dual_sound_settings":
                eventId = "Sound_Ring&vibrate_Tap";
                break;
            case "notification_ringtone":
                eventId = "Sound_NotificationSound_Tap";
                break;
            case "alarm_ringtone":
                eventId = "Sound_AlarmTones_Tap";
                break;
            case "dial_pad_tones":
                eventId = "Sound_DialpadTones_Tap";
                break;
            case "screen_locking_sounds":
                eventId = "Sound_ScreenLockingSounds_Tap";
                break;
            case "touch_sounds":
                eventId = "Sound_TouchSound_Tap";
                break;
            case "vibrate_on_touch":
                eventId = "Sound_VibrateOnTouch_Tap";
                break;
            case "boot_sounds":
                eventId = "Sound_PowerRingtone _Tap";
                break;
            case "vibration_duration":
                eventId = "Sound_HapticFeedbcak_Tap";
                break;

            // sim1 and sim2 settings
            case "sound_dual_sim1_ringtone":
                eventId = "Sound_SIM1Ringtone";
                break;
            case "sound_dual_sim1_vibrate":
                eventId = "Sound_SIM1VibrateForCalls";
                break;
            case "sound_dual_sim2_ringtone":
                eventId = "Sound_SIM2Ringtone";
                break;
            case "sound_dual_sim2_vibrate":
                eventId = "Sound_SIM2VibrateForCalls";
                break;

            //End add by ronghui.yi for task 7448330 data hub 20190218


            //account
            case "emergency_info":
            case "auto_sync_account_data":
               break;
            
            //accessibillity
            case "tts_settings_preference":
               eventId = "Settings_Accessibility_TTSSettings_Tap";
               break;
            case "font_size_preference_screen":
               eventId = "Settings_Accessibility_FontSize_Tap";
               break;
            case "magnification_preference_screen":
               eventId = "Settings_Accessibility_Magnification_Tap";
               break;
            case "daltonizer_preference_screen":
               eventId = "Settings_Accessibility_ColorCorrection_Tap";
               break;
            case "toggle_inversion_preference":
               eventId = "Settings_Accessibility_ColorInversion_Tap";
               break;
            case "toggle_large_pointer_icon":
               eventId = "Settings_Accessibility_LargeMouse_Tap";
               break;
            case "autoclick_preference_screen":
               eventId = "Settings_Accessibility_ClickPointer_Tap";
               break;
            case "toggle_power_button_ends_call_preference":
               eventId = "Settings_Accessibility_PowerEndsCall_Tap";
               break;
            case "toggle_lock_screen_rotation_preference":
               eventId = "Settings_Accessibility_AutoRotate_Tap";
               break;
            case "select_long_press_timeout_preference":
               eventId = "Settings_Accessibility_TouchHoldDelay_Tap";
               break;
            case "toggle_master_mono":
               eventId = "Settings_Accessibility_MonoAudio_Tap";
               break;
            case "captioning_preference_screen":
               eventId = "Settings_Accessibility_Captions_Tap";
               break;
            case "toggle_high_text_contrast_preference":
               eventId = "Settings_Accessibility_HighContrastText_Tap";
               break;

            //system
            case "system_update_settings":
               eventId = "Settings_System_Update_Tap";
               break;
            case "dashboard_tile_pref_com.android.settings.Settings$LanguageAndInputSettingsActivity":
               eventId = "Settings_System_Languages_Tap";
               break;
            case "dashboard_tile_pref_com.android.settings.Settings$DateTimeSettingsActivity":
               eventId = "Settings_System_Date_Tap";
               break;
            case "backup_settings":
               eventId = "Settings_System_Backup_Tap";
               break;
            case "reset_dashboard":
               eventId = "Settings_System_Regulatory_Tap";
               break;
            case "regulatory_safety_info":
               eventId = "Settings_System_Reset_Tap";
               break;               
            case "dashboard_tile_pref_com.android.settings.Settings$DeviceInfoSettingsActivity":
               eventId = "Settings_System_AboutPhone_Tap";
               break;
            //reading and eye comfort mode
            case "reading_mode_always_enable":
                eventId = "ReadingMode_Settings_Tap";
                break;
            case "reading_mode_always_enable_total":
                eventId = "ReadingMode_Vaule";
                break;
            case "reading_mode_add_apps":
                eventId = "ReadingMode_Apps_Tap";
                break;
            case "auto_schedule":
                eventId = "EyeComfort_AutoSchedule_Tap";
                break;
            case "schedule_plan":
                eventId = "EyeComfort_SchedulePlan_Tap";
                break;
            case "sunset_to_sunrise":
                eventId = "EyeComfort_SunsetToSunrise_Tap";
                break;
            case "custom_schedule":
                eventId = "EyeComfort_CustomSchedule_Tap";
                break;
            case "turn_on_now":
                eventId = "EyeComfort_TurnOnNow_Tap";
                break;
            case "ecm_ColorTemperature":
                eventId = "EyeComfort_ColorTempemperture_Tap";
                break;
            case "ecm_color_space_normal_theme":
                eventId = "EyeComfort_NormalTheme_Tap";
                break;
            case "ecm_color_space_night_theme":
                eventId = "EyeComfort_ColorSpace_Tap";
                break;
            
            //datauage
            case "restrict_background":
                eventId = "DataUsage_Datasaver_Tap";
                break;
                
            case "cellular_data_usage_1"://wifi etc is 0,need other code
                eventId = "DataUsage_SIM1CellDataUsage_Tap";
                break;

            case "cellular_data_usage_2":
                eventId = "DataUsage_SIM2CellDataUsage_Tap";
                break;
        }
        //Log.d(TAG,"subkey="+subkey+",eventId="+eventId);
        if(!eventId.equals("null")){
           mDataHub.trackEventInfo(TctDataHubConstants.TYPE_RECORD_BY_TIME,eventId);
        }   
    }
    /**
     * Trogle menu data hub
     */
    public static void  dataHubSendTroggle(Context context,String subkey,boolean onoff){
        if(isDataHubOn(context)<0){
            return;
        }      
        TctDataHubSdkManager mDataHub = TctDataHubSdkManager.getInstance(context);
        ...
        String eventId = "null";
        String status = "status"; //change to status follow doc zhixiong.liu.hz 20190219
        
        info.put(status,onoff);
                
        switch(subkey){
            case "BT":
               eventId = "Settings_Network_BT_Switch";
               break;               
            case "WIFI":
               eventId = "Settings_Network_WiFi_Switch";
               break;                      
            case "NFC":
               eventId = "Settings_Network_NFC_Switch";
               break;                  
            case "AIR":
               eventId = "Settings_Network_Airplane_Switch";
               break;
            case "PASS":
               eventId = "Settings_Security_PassVisible_Switch";
               break;
            case "emc_auto_schedule":
                eventId = "EyeComfort_AutoSchedule_Switch";
                break;
            case "DATASAVER":
                eventId = "DataUsage_Datasaver_Switch";
                break;
        }
        //Log.d(TAG,"subkey="+subkey+",eventId="+eventId+",status="+onoff);
        if(!eventId.equals("null")){
           mDataHub.trackEventInfo(TctDataHubConstants.TYPE_RECORD_BY_DATE,eventId,info);
        }  
    }
    //end add by zhixiong.liu.hz for task 7447417 data hub 20190214

    /**
     * For string type data hub each time change
     */
    public static void  dataHubSendStringsByTime(Context context,String subkey,String str){
        if(isDataHubOn(context)<0 || str == null){
            return;
        }
        TctDataHubSdkManager mDataHub = TctDataHubSdkManager.getInstance(context);
        Map<String,Object> info = new HashMap();
        ...
        info.put(str,System.currentTimeMillis());

        switch(subkey){
            case "reading_mode_apps":
                eventId = "ReadingMode_Apps";
                break;
        }
        Log.d(TAG,"subkey="+subkey+",eventId="+eventId+",str="+str);
        if(!eventId.equals("null")){
            mDataHub.trackEventInfo(TctDataHubConstants.TYPE_RECORD_BY_TIME,eventId,info);
        }
    }
    /**
     * For string type data hub each day
     */
    public static void  dataHubSendStringsByDate(Context context,String subkey,String str){
        if(isDataHubOn(context)<0 || str == null){
            return;
        }
        TctDataHubSdkManager mDataHub = TctDataHubSdkManager.getInstance(context);
        ...
        switch(subkey){
            case "emc_schedule_plan":
                eventId = "EyeComfort_SchedulePlan_Vaule";
                break;
            case "emc_custom_start_time":
                eventId = "EyeComfort_SetTurnOnAt_Vaule";
                break;
            case "emc_custom_end_time":
                eventId = "EyeComfort_SetTurnOffAt_Vaule";
                break;
            case "ecm_ColorTempemperture_Vaule":
                eventId = "EyeComfort_ColorTempemperture_Vaule";
                break;
        }
        Log.d(TAG,"subkey="+subkey+",eventId="+eventId+",str="+str);
        if(!eventId.equals("null")){
    ...
    //Begin add by ronghui.yi for task 7448330 data hub 20190218
    public static void  dataHubSoundSendStatus(Context context,String subkey,boolean onoff){
        if(isDataHubOn(context)<0){
            return;
        }
        TctDataHubSdkManager mDataHub = TctDataHubSdkManager.getInstance(context);
        ...
        switch(subkey){
            case "silent_mode_switch":
                eventId = "Sound_SilentMode_Switch";
                break;
            case "dial_pad_tones":
                eventId = "Sound_DialpadTones_Switch";
                break;
            case "screen_locking_sounds":
                eventId = "Sound_ScreenLockingSounds_Switch";
                break;
            case "touch_sounds":
                eventId = "Sound_TouchSound_Switch";
                break;
            case "vibrate_on_touch":
                eventId = "Sound_VibrateOnTouch_Switch";
                break;
            case "boot_sounds":
                eventId = "Sound_PowerRingtone_Switch";
                break;
            case "sound_dual_sim1_vibrate":
                eventId = "Sound_SIM1VibrateForCalls_Switch";
                break;
            case "sound_dual_sim2_vibrate":
                eventId = "Sound_SIM2VibrateForCalls_Switch";
                break;
        }
        //Log.d(TAG,"subkey="+subkey+",eventId="+eventId+",status="+onoff);
        if(!eventId.equals("null")){
    ...
    //Begin add by ronghui.yi for task 7448330 data hub 20190219
    public static void  dataHubVolumeStreamSendValues(Context context,int stream,int values){
        if(isDataHubOn(context)<0){
            return;
        }
        TctDataHubSdkManager mDataHub = TctDataHubSdkManager.getInstance(context);
        ...
        switch(stream){
            case AudioSystem.STREAM_MUSIC:
                eventId = "Sound_Mediavolume_Value";
                info.put("Mediavolume",values);
                break;
            case AudioSystem.STREAM_ALARM:
                eventId = "Sound_Alarmvolume_Value";
                info.put("Alarmvolume",values);
                break;
            case AudioSystem.STREAM_RING:
                eventId = "Sound_Ringvolume_Value";
                info.put("Ringvolume",values);
                break;
            case AudioSystem.STREAM_NOTIFICATION:
                eventId = "Sound_Notificationvolume_Value";
                info.put("Notificationvolume",values);
                break;
        }
//        Log.d(TAG,"stream="+stream+",eventId="+eventId+",values="+values);
        if(!eventId.equals("null")){
            mDataHub.trackEventInfo(TctDataHubConstants.TYPE_RECORD_BY_DATE, eventId, info);
        }
    }
    //End add by ronghui.yi for task 7448330 data hub 20190219
}
