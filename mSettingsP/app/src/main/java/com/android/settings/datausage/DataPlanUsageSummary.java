...
public class DataPlanUsageSummary extends DataUsageBase {
    ...
    private void updateState() {
        ...
        if (mLimitPreference != null && (info.warningLevel > 0 || info.limitLevel > 0)) {
            ...
            String warning = Utils.formatDataUsage(context, info.warningLevel);
            String limit = Utils.formatDataUsage(context, info.limitLevel);
    ...
    private static class SummaryProvider
        ...
        public void setListening(boolean listening) {
            if (listening) {
                ...
                //Begin modified by miaoliu for XRP10028370 on 2018/12/24
                if (info == null) {
                    used = Utils.formatDataUsage(mActivity, 0);
                } else if (info.limitLevel <= 0) {
                    used = Utils.formatDataUsage(mActivity, info.usageLevel);
                } else {
