    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case REQUEST_UNINSTALL:
                ...
                /// Added by yunqi.song.hz for JoyUI dualApp TASK 6766810
                //[TCT-ROM][Dual APP]Begin modified by zhiming.chen for 7432815 on 2019/1/28
                if (mUserId == PackageCloneManager.getInstance().getCloneUserId()) {
                    boolean isAppUninstalled = false;
                    try {
                        ApplicationInfo ainfo = getPackageManager().getApplicationInfoAsUser(
                                mAppEntry.info.packageName,
                                PackageManager.MATCH_DISABLED_COMPONENTS,
                                mUserId);
                        isAppUninstalled = (ainfo.flags & ApplicationInfo.FLAG_INSTALLED) == 0;
                    } catch (NameNotFoundException e) {
                        isAppUninstalled = true;
                    }
                    if (isAppUninstalled) onPackageRemoved();
                }
                //[TCT-ROM][Dual APP]End modified by zhiming.chen for 7432815 on 2019/1/28
