...
//Begin modified by jinlong.lu for XRP10029175 on 19-1-11
import java.util.HashMap;
import android.text.TextUtils;
//End modified by jinlong.lu for XRP10029175 on 19-1-11
...
public class FingerprintLaunchAppHelper {
    ...
    //Begin added by jinlong.lu for XRP10029175 on 19-1-11
    private static final boolean DEBUG = false;
    public static HashMap<String, String> sQuickLaunchShortcuts = new HashMap<>();

    static {
        sQuickLaunchShortcuts.put(FUNC_CALL_A_CONTACT, "com.tct.contacts,com.tct.dialer");
        sQuickLaunchShortcuts.put(FUNC_RECENT_CALLS, "com.tct.dialer");
        sQuickLaunchShortcuts.put(FUNC_SELFIE, "com.tcl.camera");
        sQuickLaunchShortcuts.put(FUNC_CAMERA, "com.tcl.camera");
        sQuickLaunchShortcuts.put(FUNC_START_SOUND_RECORD, "com.tcl.soundrecorder");
        sQuickLaunchShortcuts.put(FUNC_SET_TIMER, "com.google.android.deskclock");
        sQuickLaunchShortcuts.put(FUNC_CALCULATOR, "com.tct.calculator");
        sQuickLaunchShortcuts.put(FUNC_NAVIGATE_HOME, "com.google.android.apps.maps");
        sQuickLaunchShortcuts.put(FUNC_NEW_MESSAGE, "com.google.android.apps.messaging");
        sQuickLaunchShortcuts.put(FUNC_NEW_EMAIL, "com.google.android.gm");
        sQuickLaunchShortcuts.put(FUNC_ADD_CONTACT, "com.tct.contacts");
        sQuickLaunchShortcuts.put(FUNC_ADD_AGENDA, "com.google.android.calendar");
        sQuickLaunchShortcuts.put(FUNC_GOOGLE_VOICE_SEARCH, "com.google.android.googlequicksearchbox");
    }

    public static boolean isAppDisableCheck(Context context,String id) {
        try {
            String packageName = FingerprintLaunchAppHelper.sQuickLaunchShortcuts.get(id);
            if (packageName != null) {
                String[] packageArray = packageName.split(",");
                for (String p : packageArray) {
                    if (!TextUtils.isEmpty(p)) {
                        int appEnabled = context.getPackageManager()
                                .getApplicationEnabledSetting(p);
            ...
            Log.d(TAG, "check app status failed,do nothing");
            return false;
        }
        return false;
    }
    //End added by jinlong.lu for XRP10029175 on 19-1-11
    ...
    /**
     * Get fingerprint launch app info
     * @param context context is a god
     * @param fpId fingerprint  id
     * @param isCheckApp need to check app disable stutas?
     * @return launch app info {@link LaunchAppInfo}
     */
    public static LaunchAppInfo getLauncherAppInfo(Context context, int fpId,boolean isCheckApp) {
        ...
        if (fpInfos.length >= INFO_MIN_LENGHT) {
            ...
            if(isCheckApp){
                boolean targetAppIsDisable = isAppDisableCheck(context,mLaunchType);
                //app is disable,reset quick launch app function
                if (targetAppIsDisable) {
                    String type = context.getString(R.string.fingerprint_functions_unlock_screen);
                    launchAppInfo.setLaunchName(type);
                    launchAppInfo.setLaunchType(type);
