...
public class SchedulePlanSettings extends SettingsPreferenceFragment
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        if (preference == mStartTimePreference) {
            ...
            //Begin add by shilei.zhang for task 7447417 data hub 20190218
            Utils.dataHubSendStringsByDate(getContext(),"emc_schedule_plan","SunsetToSunrise");
            Utils.dataHubSendSub(getContext(),"sunset_to_sunrise");
            //End add by shilei.zhang for task 7447417 data hub 20190218
            Log.d(TAG,"mSunsetToSunrise onPreferenceTreeClick");
        }else if (preference == mCustomSchedule){
            //Begin add by shilei.zhang for task 7447417 data hub 20190218
            Utils.dataHubSendStringsByDate(getContext(),"emc_schedule_plan","CustomSchedule");
            Utils.dataHubSendSub(getContext(),"custom_schedule");
            //End add by shilei.zhang for task 7447417 data hub 20190218
        ...
        if (dialogId == DIALOG_START_TIME || dialogId == DIALOG_END_TIME) {
            ...
            return new TimePickerDialog(context, new TimePickerDialog.OnTimeSetListener() {
                ...
                public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
                    ...
                    if (dialogId == DIALOG_START_TIME) {
                        //Begin add by shilei.zhang for task 7447417 data hub 20190218
                        Utils.dataHubSendStringsByDate(getContext(),"emc_custom_start_time",time.toString());
                        //End add by shilei.zhang for task 7447417 data hub 20190218
                        mController.setCustomStartTime(time);
                    } else {
                        //Begin add by shilei.zhang for task 7447417 data hub 20190218
                        Utils.dataHubSendStringsByDate(getContext(),"emc_custom_end_time",time.toString());
                        //End add by shilei.zhang for task 7447417 data hub 20190218
    ...
    private void initRadioButtonPreferenceStatus(){
        ...
        if(autoMode == ColorDisplayController.AUTO_MODE_CUSTOM || temp == 1){
            Settings.Secure.putInt(getContentResolver(), "ecm_auto_mode", 1);
            mCustomSchedule.setChecked(true);
            mSunsetToSunrise.setChecked(false);
            mStartTimePreference.setEnabled(true);
    ...
    public int getMetricsCategory() {
        return MetricsEvent.COLOR_MODE_SETTINGS;
    }
}
