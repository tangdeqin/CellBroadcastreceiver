...
public class ApnEditor extends SettingsPreferenceFragment
    ...
    public void onCreate(Bundle icicle) {
        ...
        mTelephonyManager = (TelephonyManager) getSystemService(TELEPHONY_SERVICE);//Added by miaoliu for XRP10026955 on 2018/11/27
        ...
        if (action.equals(Intent.ACTION_EDIT)) {
            ...
            if (mFirstTime || icicle.getInt(SAVED_POS) == 0) {
                ...
                //Begin modified by miaoliu for XRP10026955 on 2018/11/28
               //Begin modified by miaoliu for XRP10026955 on 2018/11/27
                //Begin modified by miaoliu for XR7345082 on 2019/1/25
                ContentValues values = new ContentValues();
                mUri = getContentResolver().insert(uri, values);
                if(mUri == null){
                 String where = "(apn=\"\" OR apn IS NULL) AND (name=\"\" OR name IS NULL)";
                 final int result = getContentResolver().update(Telephony.Carriers.CONTENT_URI,  values, where, null);
                 if (result > 0) { //update success
                     //where = "(apn=\"\" OR apn IS NULL) AND (name=\"\" OR name IS NULL)";
                     Cursor cursor = getContentResolver().query(Telephony.Carriers.CONTENT_URI, new String[]{"_id"}, where, null, null);
                     if (cursor != null) {
                            if (cursor.getCount() > 0) {
                                cursor.moveToFirst();
                                int apnId = cursor.getInt(cursor.getColumnIndex("_id"));
                                mUri = ContentUris.withAppendedId(Telephony.Carriers.CONTENT_URI, apnId);
                                Log.d(TAG, "VALID. # matching APNs found "+cursor.getCount());
                            } else {
                                Log.d(TAG, "INVALID. # matching APNs found 0");
                            }
                            cursor.close();
                      }else{
                            Log.d(TAG, "INVALID. # matching APNs not found ");
                      }
                  }
                }
                //End modified by miaoliu for XR7345082 on 2019/1/25
                //End modified by miaoliu for XRP10026955 on 2018/11/27
                //Begin modified by miaoliu for XRP10026955 on 2018/11/28
            } else {
        ...
        //mTelephonyManager = (TelephonyManager) getSystemService(TELEPHONY_SERVICE);//Deleted by miaoliu for XRP10026955 on 2018/11/27
    ...
    public void onResume() {
        super.onResume();
          //Begin modified by miaoliu for XR7226032 on 2018/12/19
        ...
        //End added by shengjiao.liu.hz for APN REQ on 2018/09/04
         //End modified by miaoliu for XR7226032 on 2018/12/19
    ...
    private void fillUi() {
        //Begin added by miaoliu for XRP10026955 on 2018/11/27
        if(mCursor == null || mCursor.getCount() == 0){
            finish();
            return;
        }
        //End added by miaoliu for XRP10026955 on 2018/11/27
        if (mFirstTime) {
            mFirstTime = false;
        ...
        boolean isOpenMacktSim = TclUtils.isOpenMarketSim(numeric); //Modify by chenli.gao.hz for XR6998744 on 2018/09/12
    ...
    public void onSaveInstanceState(Bundle icicle) {
        ...
        //Begin deleted by miaoliu for XR7107006 on 2019/1/27
        //it will cause ANR
        //Begin modified by keyong.yu for APN REQ on 2018/07/18
        ...
        // }
        //End modified by keyong.yu for APN REQ on 2018/07/18
        //End deleted by miaoliu for XR7107006 on 2019/1/27
    }
    ...
    private boolean validateAndSave(boolean force) {
        ...
        //Begin modified by miaoliu for XR7345082 on 2019/1/27
        if (force && mNewApn && (name.length() < 1 || apn.length() < 1)) {
            //Log.d(TAG,"deleted invalid apn");
            getContentResolver().delete(mUri, null, null);
            mUri = null;
            return false;
        }
        //End modified by miaoliu for XR7345082 on 2019/1/27
