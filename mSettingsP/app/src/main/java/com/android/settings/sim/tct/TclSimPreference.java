...
//zhixiong.liu.hz remove import com.android.internal.telephony.uicc.SpnOverride;
            ...
            */
            ...
            mRadioSwith.setOnCheckedChangeListener(new OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    ...
                    */
                    //begin change by zhixiong.liu.hz for 7070928 20181112 getSubId may return null
                    int slot1SubId = SubscriptionManager.INVALID_SUBSCRIPTION_ID;
                    int slot2SubId = SubscriptionManager.INVALID_SUBSCRIPTION_ID;
                    if(SubscriptionManager.getSubId(PhoneConstants.SIM_ID_1) != null ){
                        slot1SubId = SubscriptionManager.getSubId(PhoneConstants.SIM_ID_1)[0];
                    }
                    if(SubscriptionManager.getSubId(PhoneConstants.SIM_ID_2) != null ){
                        slot2SubId = SubscriptionManager.getSubId(PhoneConstants.SIM_ID_2)[0];
                    }
                    //end change by zhixiong.liu.hz for 7070928 20181112 getSubId may return null
        ...
        final List<PhoneAccountHandle> allPhoneAccounts = telecomManager
            ...
            if (allPhoneAccounts != null && allPhoneAccounts.size() > 1) {
                //for (PhoneAccountHandle phoneAccountHandle : allPhoneAccounts) {
                    ...
                    int otherSimId = simId == 0 ? 1 : 0;
                    //begin change by zhixiong.liu.hz for 7070928 20181112 getSubId may return null
                    int slotSubId = SubscriptionManager.INVALID_SUBSCRIPTION_ID;
                    int otherSubId = SubscriptionManager.INVALID_SUBSCRIPTION_ID;
                    if(SubscriptionManager.getSubId(simId) != null){
                        slotSubId = SubscriptionManager.getSubId(simId)[0];
                    }
                    if(SubscriptionManager.getSubId(otherSubId) != null){
                        otherSubId = SubscriptionManager.getSubId(otherSubId)[0];
                    }
                    Log.d(TAG,"setToChangeDefaultCallAndSms subInfos size > 1, set to : SubId = " + otherSubId
                            + ", radioSwitchOn:" + radioSwitchOn);
                    if (!radioSwitchOn && TclInterfaceAdapter.isRadioOn(otherSubId, getContext())){
                    	...
                    	subscriptionManager.setDefaultSmsSubId(slotSubId);  
                    }
                    //end  change by zhixiong.liu.hz for 7070928 20181112 getSubId may return null
                //}
           }
