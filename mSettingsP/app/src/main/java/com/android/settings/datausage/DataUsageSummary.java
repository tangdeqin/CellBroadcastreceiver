...
public class DataUsageSummary extends DataUsageBase implements Indexable, DataUsageEditController {
    ...
    public boolean onPreferenceTreeClick(Preference preference) {        
        ...
        Utils.dataHubSendSub(getContext(),preference.getKey());
    ...
    private void addMobileSection(int subId, SubscriptionInfo subInfo) {
        ...
        CellDataPreference mMobileDataUsageTogglePreference = (CellDataPreference) findPreference(KEY_MOBILE_DATA_USAGE_TOGGLE);
        if(mMobileDataUsageTogglePreference != null){
            mMobileDataUsageTogglePreference.setClickListener(new CellDataPreference.UpdateSummaryListener() {
                @Override
                public void refreshUi() {
                    updateSubId(getContext());
                    updateState();
                }
            });
        }
        ...
        if (subInfo != null && !TextUtils.isEmpty(subInfo.getDisplayName())) {
            ...
            //Begin Modified by ben.tang NBTelCode for 7308409 on 2019/01/14
            /*
            title.setTitle(subInfo.getDisplayName());
            */
            int phoneId = SubscriptionManager.getPhoneId(subId);
            title.setTitle(getActivity().getResources()
                    .getString(R.string.two_sim_card_number_title, phoneId + 1, subInfo.getDisplayName()));
            //End Modified by ben.tang NBTelCode for 7308409 on 2019/01/14
    ...
    private void updateState() {
        ...
        if (mSummaryPreference != null) {
            ...
            if (limit <= 0) {
                ...
                mSummaryPreference.setLabels(Utils.formatDataUsage(context, 0),//Modified by miaoliu for XR7107006 on 2018/11/30
                        Utils.formatDataUsage(context, limit));//Modified by miaoliu for XR7107006 on 2018/11/30
        ...
        if (mLimitPreference != null && (info.warningLevel > 0 || info.limitLevel > 0)) {
            String warning = Utils.formatDataUsage(context, info.warningLevel);//Modified by miaoliu for XR7107006 on 2018/11/30
            String limit = Utils.formatDataUsage(context, info.limitLevel);//Modified by miaoliu for XR7107006 on 2018/11/30
    ...
    private static class SummaryProvider
        ...
        public void setListening(boolean listening) {
            if (listening) {
                ...
                String used;
                if (info == null) {
                    used = Utils.formatDataUsage(mActivity, 0);//Modified by miaoliu for XR7107006 on 2018/11/30
                } else if (info.limitLevel <= 0) {
                    used = Utils.formatDataUsage(mActivity, info.usageLevel);//Modified by miaoliu for XR7107006 on 2018/11/30
                } else {
    ...
    public static final Indexable.SearchIndexProvider SEARCH_INDEX_DATA_PROVIDER =
       ...
       //End added by miaoliu for XR7107006 on 2018/11/15
//begin tao.ning.sz for XR P10029069 20190108
    public void updateSubId(Context mContext){
        ///reCreate UI if default data id changed.
        int newDefaultSubId = DataUsageUtils.getDefaultSubscriptionId(mContext);

        mDefaultTemplate = DataUsageUtils.getDefaultTemplate(mContext, newDefaultSubId);
    }
//end tao.ning.sz for XR P10029069 20190108
}
