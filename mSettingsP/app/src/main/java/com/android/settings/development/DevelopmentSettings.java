...
public class DevelopmentSettings extends RestrictedSettingsFragment
    ...
    private void updateDebugHwRendererOptions() {
        String value = SystemProperties.get("debug.hwui.renderer");
        if (value == null) {
            value = "";
        }
    ...
    private void writeDebugHwRendererOptions(Object newValue) {
        SystemProperties.set("debug.hwui.renderer",
                newValue == null ? "" : newValue.toString());
        pokeSystemProperties();
        updateDebugHwRendererOptions();
    }
    ...
    private void updateWifiAggressiveHandoverOptions() {
        boolean enabled = false;//mWifiManager.getAggressiveHandover() > 0; //zhixiong.liu may check
        updateSwitchPreference(mWifiAggressiveHandover, enabled);
    }

    private void writeWifiAggressiveHandoverOptions() {
        //mWifiManager.enableAggressiveHandover(mWifiAggressiveHandover.isChecked() ? 1 : 0);//zhixiong.liu may check
    }

    private void updateWifiAllowScansWithTrafficOptions() {
//        boolean enabled = mWifiManager.getAllowScansWithTraffic() > 0;//zhixiong.liu may check
//        updateSwitchPreference(mWifiAllowScansWithTraffic, enabled);//zhixiong.liu may check
    }

    private void writeWifiAllowScansWithTrafficOptions() {
//        mWifiManager.setAllowScansWithTraffic(mWifiAllowScansWithTraffic.isChecked() ? 1 : 0);//zhixiong.liu may check
    }
    ...
    private void updateUsbConfigurationValues() {
        if (mUsbConfiguration != null) {
           ...
           //Begin modified by miaoliu for 7216798 on 2018/12/10 
            String[] values = getResources().getStringArray(R.array.usb_configuration_values_new);
            String[] titles = getResources().getStringArray(R.array.usb_configuration_titles_new);
            //End modified by miaoliu for 7216798 on 2018/12/10 
            int index = 0;
            for (int i = 0; i < titles.length; i++) {
    ...
    private void updateBluetoothA2dpConfigurationValues() {
        ...
        synchronized (mBluetoothA2dpLock) {
            if (mBluetoothA2dp != null) {
                //Begin modified by weijun.pan for XR7107569 on 2018/11/10
                //codecStatus = null;//mBluetoothA2dp.getCodecStatus();//zhixiong.liu may check
                codecStatus = mBluetoothA2dp.getCodecStatus(null);
                //End modified by weijun.pan for XR7107569 on 2018/11/10
                if (codecStatus != null) {
    ...
    private void writeBluetoothConfigurationOption(Preference preference,
        ...
        synchronized (mBluetoothA2dpLock) {
            if (mBluetoothA2dp != null) {
//                mBluetoothA2dp.enableOptionalCodecs();//zhixiong.liu may check
                //Begin adde by weijun.pan for XR7107569 on 2018/11/10
                mBluetoothA2dp.enableOptionalCodecs(null);
                //End adde by weijun.pan for XR7107569 on 2018/11/10
            }
        }
        return;
        case 7:
        synchronized (mBluetoothA2dpLock) {
            if (mBluetoothA2dp != null) {
//                mBluetoothA2dp.disableOptionalCodecs();//zhixiong.liu may check
                //Begin adde by weijun.pan for XR7107569 on 2018/11/10
        ...
        synchronized (mBluetoothA2dpLock) {
            if (mBluetoothA2dp != null) {
//                mBluetoothA2dp.setCodecConfigPreference(codecConfig);//zhixiong.liu may check
                //Begin adde by weijun.pan for XR7107569 on 2018/11/10
