...
import android.os.SystemProperties;// [FEATURE]-Add-BEGIN by TSNJ,shu.wang,11/14/2014,PR-722787,
...
public class CBLinkify {
    ...
    // [FEATURE]-Add-BEGIN by TSNJ,shu.wang,10/30/2014,PR-722787,
    private static final int PHONE_NUMBER_MINIMUM_DIGITS_ISRAEL = 3;
    private static final int PHONE_NUMBER_AST_MIN_DIGITS = 2;
    // [FEATURE]-Add-END by TSNJ,shu.wang,
    ...
    public static final MatchFilter sPhoneNumberMatchFilter = new MatchFilter() {
        public final boolean acceptMatch(CharSequence s, int start, int end) {
            ...
            // [FEATURE]-Add-BEGIN by TSNJ,shu.wang,10/30/2014,PR-722787,
            Boolean isUseIsraelMatch = (SystemProperties.get("def.israel.on","false")).equals("true");
            android.util.Log.i("yfj","s="+s.toString().trim());
            android.util.Log.i("yfj","isUseIsraelMatch="+isUseIsraelMatch);
            android.util.Log.i("yfj","start="+start+" end="+end);
                ...
                if (Character.isDigit(s.charAt(i))) {
                    ...
                    //if (digitCount >= PHONE_NUMBER_MINIMUM_DIGITS) {
                        ...
                        if(!isUseIsraelMatch){
                           ...
                           if (digitCount >= PHONE_NUMBER_MINIMUM_DIGITS_ISRAEL) {
                               return true;
                           } else if (s.charAt(start) == '*' && digitCount >= PHONE_NUMBER_AST_MIN_DIGITS) {
                              return true;
                          }
                   // [FEATURE]-Add-END by TSNJ,shu.wang,
                    }
    ...
    public static final TransformFilter sPhoneNumberTransformFilter = new TransformFilter() {
        ...
        if (!isUseIsraelMatch) {
            ...
            return TctWrapperManager.tct_digitsAsteriskAndPlusOnly(match); // MODIFIED by yuxuan.zhang, 2016-06-13,BUG-1112693
        }
        // [FEATURE]-Add-END by TSNJ,shu.wang,
        }
    };
    ...
    public static final boolean addLinks(Spannable text, int mask) {
        ...
        if ((mask & MAP_ADDRESSES) != 0) {
            ...
            gatherLinks(links, text, TctWrapperManager.getIsTctPhoneIsrael(),new String[] { "tel:" },sPhoneNumberMatchFilter, sPhoneNumberTransformFilter);
            // [FEATURE]-Add-END by TSNJ,shu.wang,
        }

        pruneOverlaps(links);

        android.util.Log.i("yfj","links.size()="+links.size());
        if (links.size() == 0) {
            return false;
        }

        for (LinkSpec link: links) {
            android.util.Log.i("yfj","text="+text);
